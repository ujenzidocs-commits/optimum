...
const steps = [
  { icon: PhoneCall, num: '01', title: 'Free Consultation', desc: 'Tell us about your business. We\'ll analyze your needs and recommend the right Tally Prime solution — Silver, Gold, Plus, Enterprise, or Custom TDL.', color: 'from-accent to-blue-400' },
    ...
    <section className="py-24 bg-white dark:bg-navy-950">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <span className="inline-block rounded-full bg-accent/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-accent">How It Works</span>
          <h2 className="mt-4 text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white">
            From Zero to Tally Prime in <span className="text-accent">4 Simple Steps</span>
          </h2>
        ...
        <div className="mt-16 relative">
          ...
          <div className="hidden lg:block absolute top-24 left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-accent/20 via-accent/40 to-accent/20" />
            ...
            {steps.map((step, i) => {
              ...
              return (
                <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <p className="mt-2 text-sm text-navy-600 dark:text-navy-400 leading-relaxed">{step.desc}</p>
                </motion.div>
              );
            })}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <a href="#contact" className="rounded-xl bg-accent px-8 py-4 text-sm font-bold text-white shadow-xl shadow-accent/30 hover:bg-accent-dark transition-all hover:scale-105">
                Request Free Demo
              </a>
