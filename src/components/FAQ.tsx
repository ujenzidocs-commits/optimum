  ...
  return (
    <section id="faq" className="py-24 bg-white dark:bg-navy-950">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2">{cats.map(c=><button key={c} onClick={()=>setActiveCat(c)} className={`rounded-lg px-3.5 py-2 text-xs font-medium transition ${activeCat===c?'bg-navy-900 dark:bg-yellow-500 text-white':'bg-navy-100 dark:bg-navy-800 text-navy-600 dark:text-navy-300 hover:bg-navy-200'}`}>{c}</button>)}</div>
        <div className="mt-6 space-y-2.5">{filtered.map(faq=>(
          <div key={faq.id} className={`rounded-xl border transition-all ${openId===faq.id?'border-yellow-300/30 bg-white dark:bg-navy-800 shadow-md':'border-navy-100 dark:border-navy-700 bg-navy-50/50 dark:bg-navy-800/50 hover:border-navy-200'}`}>
            <button onClick={()=>setOpenId(openId===faq.id?null:faq.id)} className="flex w-full items-center gap-3 p-5 text-left">
              <HelpCircle className={`h-5 w-5 shrink-0 ${openId===faq.id?'text-yellow-600':'text-navy-600'}`}/>
              <span className="flex-1 text-sm font-semibold text-navy-900 dark:text-white">{faq.q}</span>
              <ChevronDown className={`h-4 w-4 text-navy-600 transition-transform ${openId===faq.id?'rotate-180':''}`}/>
            </button>
            <AnimatePresence>{openId===faq.id&&<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden">
              <div className="px-5 pb-5 pl-13 text-sm text-navy-600 dark:text-navy-700 leading-relaxed">{faq.a}<span className="mt-2 inline-block rounded-full bg-navy-100 dark:bg-navy-700 px-2 py-0.5 text-[10px] text-navy-500 ml-2">{faq.cat}</span></div>
