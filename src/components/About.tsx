              ...
              <div className="space-y-7">
                {[{icon:Target,title:'Our Mission',text:c.mission},{icon:Eye,title:'Our Vision',text:c.vision},{icon:Zap,title:'Why Choose Us',text:'Certified Tally experts, rapid implementation, 24/7 support, and solutions customized for the Kenyan market.'}].map(({icon:Ic,title,text})=>(
                  <div key={title} className="flex items-start gap-4">
                    <div className="h-11 w-11 rounded-xl bg-yellow-400/20 flex items-center justify-center shrink-0"><Ic className="h-5 w-5 text-yellow-600"/></div>
                    <div><h4 className="text-base font-bold text-white">{title}</h4><p className="mt-1 text-sm text-navy-700 leading-relaxed">{text}</p></div>
                  </div>
                ))}
              </div>
            </div>
